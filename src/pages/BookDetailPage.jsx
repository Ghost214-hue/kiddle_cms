...
// ─────────────────────────────────────────────
const MOCK_BOOKS = [
  { _id:'b1', title:'The Midnight Library', author:'Matt Haig', price:24.99, salePrice:null, rating:4.4, reviewCount:1240, ageRange:'Young Adult', badge:'#1 Bestseller', badgeColor:'#8a6030', slug:'the-midnight-library', categories:['fiction','young-adults','bestsellers'], img:'https://images.unsplash.com/photo-1481627834876-b7833e8f5570?w=500&q=80', description:`Between life and death there is a library, and within that library, the shelves go on forever. Every book provides a chance to try another life you could have lived. To see how things would be if you had made other choices...\n\nWould you have done anything different, if you had the chance to undo your regrets?\n\nNora Seed finds herself in the Midnight Library. Until she decides to live the life of her dreams, she will keep visiting this magical realm of possibilities.`, pageCount:304, publisher:'Viking', publishedDate:'August 13, 2020', isbn:'978-0-525-55947-4', language:'English', formats:['Hardcover','Paperback','E-book','Audiobook'] },
  { _id:'b2', title:'Before the Coffee Gets Cold', author:'Toshikazu Kawaguchi', price:18.99, salePrice:null, rating:4.8, reviewCount:876, ageRange:'Adult', badge:'New', badgeColor:'#2d7a4f', slug:'before-the-coffee-gets-cold', categories:['fiction','bestsellers'], img:'https://images.unsplash.com/photo-1544716278-ca5e3f4abd8c?w=500&q=80', description:`In a small back alley in Tokyo, there is a café which has been serving carefully brewed coffee for more than one hundred years. But this coffee shop offers its customers a unique experience: the chance to travel back in time.`, pageCount:213, publisher:'Hanover Square Press', publishedDate:'November 17, 2020', isbn:'978-1-335-43198-7', language:'English', formats:['Hardcover','Paperback','E-book'] },
  { _id:'b3', title:'Where the Crawdads Sing', author:'Delia Owens', price:23.00, salePrice:null, rating:4.7, reviewCount:2100, ageRange:'Adult', badge:null, badgeColor:null, slug:'where-the-crawdads-sing', categories:['fiction','bestsellers'], img:'https://images.unsplash.com/photo-1512820790803-83ca734da794?w=500&q=80', description:`For years, rumors of the "Marsh Girl" have haunted Barkley Cove, a quiet town on the North Carolina coast. So in late 1969, when handsome Chase Andrews is found dead, the locals immediately suspect Kya Clark, the so-called Marsh Girl.`, pageCount:368, publisher:"G.P. Putnam's Sons", publishedDate:'August 14, 2018', isbn:'978-0-735-21909-0', language:'English', formats:['Hardcover','Paperback','E-book','Audiobook'] },
  { _id:'b9', title:'Things Fall Apart', author:'Chinua Achebe', price:19.99, salePrice:null, rating:4.9, reviewCount:3200, ageRange:'Adult', badge:'#1 Bestseller', badgeColor:'#8a6030', slug:'things-fall-apart', categories:['fiction','african-writers','bestsellers'], img:'https://images.unsplash.com/photo-1589829085413-56de8ae18c73?w=500&q=80', description:`A classic novel that depicts the life of Okonkwo, a leader and local wrestling champion in Umuofia. It explores the clash between traditional Igbo culture and the forces of European colonialism.`, pageCount:209, publisher:'Heinemann', publishedDate:'1958', isbn:'978-0-385-47454-2', language:'English', formats:['Paperback','E-book','Audiobook'] },
  { _id:'cbc1', title:"Mathematics Learner's Book Grade 7", author:'KICD', price:850, salePrice:765, rating:4.7, reviewCount:245, ageRange:'12-15 Years', badge:'CBC Approved', badgeColor:'#D97706', slug:'mathematics-learner-grade-7', categories:['cbc-education','junior-secondary','mathematics'], img:'https://images.unsplash.com/photo-1503676260728-1c00da094a0b?w=500&q=80', description:`Complete coverage of the Grade 7 mathematics curriculum with clear explanations, examples, and practice exercises aligned with CBC requirements.`, pageCount:342, publisher:'Kenya Institute of Curriculum Development', publishedDate:'January 2023', isbn:'978-9966-34-567-8', language:'English', formats:['Paperback'] },
]
...
function generateMockReviews(bookId, rating, reviewCount) {
  ...
  return Array.from({ length: count }, (_, i) => ({
    ...
    rating: Math.round((Math.min(5, Math.max(4, rating + (Math.random()-0.5)*0.8))) * 2) / 2,
...
function getRatingBreakdown(rating) {
  ...
  return [5,4,3,2,1].map((stars, i) => ({ stars, pct: Math.round((v[i]/total)*100) }))
}
    ...
    <div className="bg-[rgba(255,255,255,0.52)] border border-[rgba(200,170,130,0.28)] rounded-2xl p-[18px_20px] backdrop-blur-sm transition-all duration-200 hover:shadow-md">
      <div className="flex items-center justify-between mb-2.5">
        <div className="flex items-center gap-2.5">
          <div className="w-9 h-9 rounded-full flex items-center justify-center text-[14px] font-bold text-white"
            style={{ background: `hsl(${review.id * 47}, 45%, 75%)` }}>
            {review.name[0]}
          </div>
          <div>
            <div className="text-[13px] font-semibold text-[#3d2010]">{review.name}</div>
...
function BookCover({ book }) {
  ...
  const src = book.img || book.coverImage
  const fallbackIcon = book.type === 'stationery' ? '✏️' : book.type === 'accessory' ? '🎒' : '📖'

  if (!imgErr && src) {
    return (
      <img src={src} alt={book.title} onError={() => setImgErr(true)}
  ...
  return (
    <div className="w-full h-full flex items-center justify-center"
      ...
      <div className="w-[55%] h-[75%] bg-gradient-to-br from-white/12 to-white/04 rounded-[4px_12px_12px_4px] border border-white/20 shadow-[8px_12px_40px_rgba(0,0,0,0.3)] flex flex-col items-center justify-center p-5 relative">
        ...
        <div className="text-[9px] text-white/60">{book.author || book.brand}</div>
      </div>
    </div>
  )
}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-[56px]">
          ...
          <div className="space-y-4 pt-4">
            ...
            <div className="space-y-2 pt-4">
              {[...Array(5)].map((_,i) => (
                <div key={i} className="h-3 bg-[#ede5d8] rounded animate-pulse" style={{ width:`${85-i*8}%` }}/>
              ))}
            </div>
          </div>
        </div>
...
export default function BookDetailPage() {
  ...
  useEffect(() => {
    if (!book) return
    setReviews(generateMockReviews(book._id, book.rating ?? 4.0, book.reviewCount ?? 100))
    setRatingBreakdown(getRatingBreakdown(book.rating ?? 4.0))
  }, [book])
    ...
    return (
      <div className="bg-[#f5f0e8] min-h-screen pt-20 flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="font-['Playfair_Display',serif] text-2xl text-[#3d2010] mb-4">Item not found</h2>
          <Link to="/books" className="text-[#a0693a] no-underline hover:underline">← Back to Shop</Link>
        </div>
      </div>
    )
  }
  ...
  function handleAddToCart() {
    addToCart({ ...book, price: finalPrice, format: isBook ? selectedFormat : undefined }, qty)
    setAdded(true)
    ...
    <div className="bg-[#f5f0e8] min-h-screen pt-[68px]">
      ...
      {book._id === 'b1' && (
        <div className="bg-[rgba(160,105,58,0.10)] border-b border-[rgba(180,140,90,0.20)] py-2.5 px-10 flex items-center justify-center gap-2.5 flex-wrap">
          ...
          <span className="text-[12.5px] text-[#5c3d1e] font-medium">LIMITED EDITION SIGNED COPIES:</span>
      ...
      <div className="max-w-[1280px] mx-auto px-[clamp(16px,4vw,40px)] py-8">
        ...
        <div className="flex items-center gap-2 text-xs text-[#9a7a5a] mb-8 flex-wrap">
          ...
          <Link to={productBackHref(book)} className="text-[#9a7a5a] no-underline hover:text-[#a0693a]">{typeName}</Link>
          <span className="text-[#c4a882]">›</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-[56px] mb-16 items-start">
          ...
          <div>
            <div className="relative rounded-[20px] overflow-hidden aspect-[3/4] max-h-[520px] shadow-[0_24px_64px_rgba(100,60,20,0.20)]">
              ...
              {book.badge && (
                <div className="absolute top-4 left-4 bg-[rgba(160,105,58,0.15)] border border-[rgba(160,105,58,0.35)] backdrop-blur-md rounded-xl py-[5px] px-3 text-[10.5px] font-bold text-[#7a4e22]">
                  {book.badge}
                </div>
              )}

              <button className="absolute bottom-4 right-4 w-9 h-9 rounded-full bg-white/75 backdrop-blur-sm border border-[rgba(180,140,90,0.3)] flex items-center justify-center cursor-pointer hover:bg-white/90 hover:scale-105 transition-all duration-200">
          ...
          <div>
            <div className="flex items-center gap-2 text-[11.5px] text-[#a0693a] font-semibold tracking-[0.04em] uppercase mb-2.5 flex-wrap">
              ...
              <span className="text-[#9a7a5a] font-normal normal-case tracking-normal">
                {isBook ? `${selectedFormat} Edition` : book.brand || 'Kiddle'}
              </span>
            </div>
            ...
            <p className="text-[15px] text-[#9a7a5a] italic mb-3.5">
              {isBook ? 'by' : 'from'} <span className="text-[#a0693a] not-italic">{book.author || book.brand || 'Kiddle'}</span>
            </p>
            ...
            {book.rating && (
              <div className="flex items-center gap-3 mb-5 flex-wrap">
                ...
                <span className="text-[13px] text-[#a0693a] font-semibold">{book.rating}</span>
            ...
            <div className="flex items-baseline gap-3 mb-6 flex-wrap">
              ...
              {book.salePrice && (
                <>
                  <span className="text-base text-[#b8998a] line-through">{displayPrice(book.price)}</span>
            ...
            {isBook && book.formats?.length > 0 && (
              <div className="mb-6">
                <div className="text-[11px] font-semibold text-[#5c3d1e] uppercase tracking-[0.07em] mb-2.5">Select Format</div>
                <div className="flex gap-2 flex-wrap">
                  {book.formats.map(fmt => (
                    <button key={fmt} onClick={() => setSelectedFormat(fmt)}
                      ...
                      style={{
                        borderColor: selectedFormat===fmt ? '#a0693a' : 'rgba(180,140,90,0.3)',
                        background: selectedFormat===fmt ? 'rgba(160,105,58,0.14)' : 'rgba(255,255,255,0.6)',
            ...
            {/* Quantity */}
            <div className="mb-6">
              <div className="text-[11px] font-semibold text-[#5c3d1e] uppercase tracking-[0.07em] mb-2.5">Quantity</div>
            ...
            <div className="flex gap-3 mb-5 flex-wrap">
              <button onClick={handleAddToCart}
                className={`flex-1 min-w-[180px] flex items-center justify-center gap-2 py-[14px] px-7 rounded-[28px] text-[14px] font-semibold cursor-pointer transition-all duration-250 ${added ? 'bg-[rgba(60,140,80,0.15)] border-[1.5px] border-[rgba(60,140,80,0.35)] text-[#2d7a45] shadow-none' : 'bg-gradient-to-br from-[#c48b52] to-[#a0693a] text-white shadow-[0_6px_24px_rgba(160,105,58,0.35)] hover:shadow-[0_8px_28px_rgba(160,105,58,0.4)] hover:-translate-y-px'}`}>
                {added ? (
              ...
              <button onClick={() => toggleWishlist(book)}
                className={`w-12 h-12 rounded-full flex items-center justify-center cursor-pointer transition-all duration-200 border-[1.5px] hover:scale-105 ${wishlisted ? 'bg-[rgba(160,105,58,0.15)] border-[#a0693a]' : 'bg-[rgba(255,255,255,0.7)] border-[rgba(180,140,90,0.35)] hover:bg-[rgba(255,255,255,0.9)]'}`}>
                <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
                  <path d="M9 15.5C9 15.5 2 11 2 6.5C2 4.3 3.8 2.5 6 2.5C7.4 2.5 8.6 3.3 9 4.2C9.4 3.3 10.6 2.5 12 2.5C14.2 2.5 16 4.3 16 6.5C16 11 9 15.5 9 15.5Z"
                    fill={wishlisted ? '#a0693a' : 'none'} stroke="#a0693a" strokeWidth="1.4"/>
                </svg>
              </button>
            </div>
            ...
            <div className="flex gap-5 flex-wrap pt-3.5 border-t border-[rgba(180,140,90,0.18)]">
              <div className="flex items-center gap-1.5 text-xs text-[#7a5c3a]"><span className="text-sm">🚚</span> Ships in 24 hours</div>
              {isBook && <div className="flex items-center gap-1.5 text-xs text-[#7a5c3a]"><span className="text-sm">📖</span> Free chapter preview</div>}
              {book.ageRange && <div className="flex items-center gap-1.5 text-xs text-[#7a5c3a]"><span className="text-sm">🎯</span> Ages {book.ageRange}</div>}
              {!isBook && book.inStock !== false && <div className="flex items-center gap-1.5 text-xs text-[#7a5c3a]"><span className="text-sm">✓</span> In stock</div>}
            </div>
          </div>
        </div>
          ...
          <div className="flex border-b border-[rgba(180,140,90,0.2)] mb-7 gap-0 flex-wrap">
            {['description','details','reviews'].map(t => (
              <button key={t} onClick={() => setTab(t)}
                ...
                {t}{t==='reviews' ? ` (${(book.reviewCount||0).toLocaleString()})` : ''}
              </button>
            ))}
          </div>
          ...
          {tab === 'description' && (
            <div className="max-w-[720px]">
              {(book.description || 'No description available.')
                .split('\n\n')
                .map((para, i) => (
                  <p key={i} className="text-[14px] text-[#5c3d1e] leading-[1.8] mb-4">{para}</p>
                ))}
            </div>
          )}
          ...
          {tab === 'details' && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-w-[720px]">
              {detailRows.map(([label, value]) => (
                <div key={label} className="bg-[rgba(255,255,255,0.52)] border border-[rgba(200,170,130,0.25)] rounded-xl p-3 backdrop-blur-sm hover:shadow-sm transition-all duration-200">
                  <div className="text-[10px] text-[#9a7a5a] font-semibold uppercase tracking-[0.06em] mb-1">{label}</div>
                  <div className="text-[13.5px] text-[#3d2010] font-medium">{value}</div>
                </div>
              ))}
            </div>
          )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_2fr] gap-10 items-start">
              ...
              <div className="bg-[rgba(255,255,255,0.52)] border border-[rgba(200,170,130,0.28)] rounded-[20px] p-6 backdrop-blur-sm sticky top-[84px]">
                <div className="text-center mb-5">
                  ...
                  {book.rating && <StarRating rating={book.rating} size="lg" showCount={false}/>}
                ...
                {ratingBreakdown.map(({ stars, pct }) => (
                  <div key={stars} className="flex items-center gap-2.5 mb-2">
                    <span className="text-[11px] text-[#7a5c3a] w-[10px]">{stars}</span>
                    <span className="text-[11px] text-[#a0693a]">★</span>
                    <div className="flex-1 h-1.5 bg-[rgba(160,105,58,0.12)] rounded-full overflow-hidden">
                      <div className="h-full bg-[#a0693a] rounded-full transition-all duration-600" style={{ width:`${pct}%` }}/>
                    </div>
                    <span className="text-[11px] text-[#9a7a5a] w-[28px] text-right">{pct}%</span>
                  </div>
                ))}
              </div>
