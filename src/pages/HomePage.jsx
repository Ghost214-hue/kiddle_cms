...
function HeroCarousel({ slides = DEFAULT_HERO_SLIDES }) {
  ...
  function go(n) { setCur((n + slides.length) % slides.length) }
  ...
  return (
    <div className="relative rounded-[28px] overflow-hidden shadow-xl border border-white/80 transition-all duration-500"
      ...
      <div className="relative h-[200px] sm:h-[220px] md:h-[240px] overflow-hidden">
        {!imgErr[cur] && (
          <img key={cur} src={s.img} alt={s.title}
            onError={() => setImgErr(p => ({ ...p, [cur]: true }))}
        ...
        <div className="absolute top-3.5 left-3.5 text-[9.5px] font-extrabold py-1 px-3 rounded-full shadow-sm tracking-wide"
      ...
      <div className="flex justify-center gap-1.5 pb-4 sm:pb-5">
        {slides.map((_, i) => (
          <button key={i} onClick={() => setCur(i)} className="h-1 rounded-full transition-all duration-300"
            style={{ width: cur === i ? '22px' : '5px', background: cur === i ? s.accent : 'rgba(0,0,0,0.18)' }} />
        ))}
      </div>
      ...
      <style>{`@keyframes ks-fadein-slide { from{opacity:0;transform:translateY(10px)} to{opacity:1;transform:translateY(0)} }`}</style>
    </div>
  )
}
...
export default function HomePage() {
  ...
  return (
    <div className="bg-[#faf7f2] min-h-screen pt-20 md:pt-20">
      ...
      {/* 5. FEATURED PICKS */}
      <FadeSection className="px-5 md:px-8 max-w-7xl mx-auto py-14 md:py-16">
        <FeaturedCarousel title="⭐ Bestsellers This Month" viewAllHref="/books/bestsellers" />
      </FadeSection>
      ...
      <FadeSection className="px-5 md:px-8 max-w-7xl mx-auto py-14 md:py-16">
        <div className="bg-gradient-to-br from-[#FEF9EC] to-[#FEF3C7] border border-[rgba(252,211,77,0.5)] rounded-2xl md:rounded-3xl p-8 md:p-12 text-center shadow-md">
          ...
          <p className="text-[11px] md:text-xs text-[#B45309] opacity-70 mt-4">No spam. Only great books.</p>
        </div>
      </FadeSection>
    </div>
  )
